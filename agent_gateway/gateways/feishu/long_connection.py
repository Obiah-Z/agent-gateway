...
该运行时通过 `lark-cli event consume` 订阅事件流，把实时事件转换成统一的 InboundMessage
后送进 channel runtime。每个 event key 都对应一个独立消费者进程，便于隔离和重启。
"""
...
from agent_gateway.runtime.observability.events import new_correlation_id
...
class FeishuLongConnectionConsumer:
    """一个 event key 对应的独立 lark-cli 消费进程。"""
...
class FeishuLongConnectionRuntime:
    ...
    def _run_consumer_once(self, consumer: FeishuLongConnectionConsumer) -> None:
        """拉起一次 lark-cli event consume 进程并处理其 stdout/stderr。"""
    ...
    def _submit_event(self, event: dict[str, Any], account_id: str) -> None:
        """给拦截器一个机会消费事件，例如 onboarding 自动绑定。"""
    ...
    def _parse_event_line(line: str) -> dict[str, Any] | None:
        """解析 lark-cli stdout 中的单行 JSON 事件。"""
    ...
    def _process_env(consumer: FeishuLongConnectionConsumer) -> dict[str, str]:
        """把 account.config 中的 event_env_* 映射进子进程环境变量。"""
    ...
    def _normalize_event_keys(config: dict[str, Any]) -> tuple[str, ...]:
        """把单个 event_key 或多个 event_keys 统一成去重后的序列。"""
