...
from agent_gateway.runtime.domain.models import Binding, InboundMessage
...
class GatewayServer:
    ...
    async def _m_diet_recent(self, params: dict[str, Any]) -> dict[str, Any]:
        """处理控制面 最近饮食记录 RPC 请求。"""
        if self.control_plane is None:
            raise RuntimeError("control plane not configured")
        return self.control_plane.recent_diet(
    ...
    async def _m_tasks_lanes_recovery(self, params: dict[str, Any]) -> dict[str, Any]:
        """处理控制面 session lane 恢复建议 RPC 请求。"""
        if self.control_plane is None:
            raise RuntimeError("control plane not configured")
    ...
    async def _m_tasks_lanes_recovery_execute(self, params: dict[str, Any]) -> dict[str, Any]:
        """处理控制面 session lane 批量恢复执行 RPC 请求。"""
        if self.control_plane is None:
            raise RuntimeError("control plane not configured")
        return self.control_plane.execute_session_lane_recovery(
            limit=int(params.get("limit", 50)),
            execute=bool(params.get("execute", False)),
        )

    async def _m_tasks_lanes_recovery_events(self, params: dict[str, Any]) -> dict[str, Any]:
        """处理控制面 session lane 恢复审计事件 RPC 请求。"""
        if self.control_plane is None:
            raise RuntimeError("control plane not configured")
        return self.control_plane.lane_recovery_events(
            limit=int(params.get("limit", 50)),
            session_key=str(params.get("session_key", "")),
            worker_id=str(params.get("worker_id", "")),
            event_type=str(params.get("type", "")),
    ...
    async def _m_tasks_lanes_release(self, params: dict[str, Any]) -> dict[str, Any]:
        """处理控制面 session lane 释放 RPC 请求。"""
        if self.control_plane is None:
            raise RuntimeError("control plane not configured")
        return self.control_plane.release_session_lane(
            session_key=str(params.get("session_key", "")),
