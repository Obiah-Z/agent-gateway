...
def register_builtin_tools(
    ...
    *,
    ...
    def format_task_plan_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 task_plan_gate_review JSON 转成适合 reviewer 直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "task_plan_gate_review":
            return "Error: gate_review_json type must be task_plan_gate_review"
        ...
        sections = [
            "## 计划门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名计划'}",
            f"- 边界：{data.get('note') or '这是执行前门禁，不代表已经完成发布验证。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
        ...
        if risks:
            sections.extend(["", "## 主要风险", _markdown_bullets(risks)])
        if include_passed_items:
            sections.extend(["", "## 已通过项", _markdown_table(["检查项", "状态", "依据"], passed_rows)])
        return "\n".join(sections).strip()
    ...
    def format_agent_collaboration_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 collaboration_gate_review JSON 转成适合 reviewer 直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "collaboration_gate_review":
            return "Error: gate_review_json type must be collaboration_gate_review"
        ...
        decision = str(data.get("decision") or "").strip()
        agents = _clean_strings(data.get("agents") if isinstance(data.get("agents"), list) else [])
        sections = [
            "## 协作路线门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名协作路线'}",
            f"- 参与 Agent：{', '.join(agents) if agents else '待明确'}",
            f"- 边界：{data.get('note') or '这是协作路线门禁，不代表任何 Agent 已经执行。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
    ...
    def format_agent_handoff_package_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 agent_handoff_package_gate_review JSON 转成可直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "agent_handoff_package_gate_review":
        ...
        sections = [
            "## Agent 交接包门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 目标 Agent：{data.get('review_target') or '待明确'}",
            f"- 边界：{data.get('note') or '这是交接包门禁，不代表目标 Agent 已经执行。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
    ...
    def format_collaboration_progress_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 collaboration_progress_gate_review JSON 转成可直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "collaboration_progress_gate_review":
        ...
        next_stage = data.get("next_stage") if isinstance(data.get("next_stage"), dict) else {}
        next_stage_label = "已完成，无下一阶段"
        if next_stage:
            step = next_stage.get("step") or "?"
            agent_id = next_stage.get("agent_id") or "待明确"
            next_stage_label = f"第 {step} 阶段 / {agent_id}"
        ...
        sections = [
            "## 协作进度门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名协作进度'}",
            f"- 当前进度：{completed}/{total}",
            f"- 下一阶段：{next_stage_label}",
            f"- 边界：{data.get('note') or '这是协作进度门禁，不代表任何 Agent 已经执行。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
    ...
    def format_research_evidence_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 research_evidence_gate_review JSON 转成可直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "research_evidence_gate_review":
            return "Error: gate_review_json type must be research_evidence_gate_review"
        ...
        sections = [
            "## Research 证据门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名证据包'}",
            f"- 证据质量：{data.get('evidence_quality') or 'missing'}",
            f"- 来源数量：{data.get('source_count', 0)}",
            f"- 一手来源：{data.get('primary_source_count', 0)}",
            f"- 边界：{data.get('note') or '这是证据复用前门禁，不代表事实永久有效。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
        ...
        risks = _clean_strings(data.get("risks") if isinstance(data.get("risks"), list) else [])
        if risks:
            sections.extend(["", "## 不确定点和风险", _markdown_bullets(risks)])
        if include_passed_items:
            sections.extend(["", "## 已通过项", _markdown_table(["检查项", "状态", "依据"], passed_rows)])
        return "\n".join(sections).strip()
    ...
    def format_research_option_comparison_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 research_option_comparison_gate_review JSON 转成可直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "research_option_comparison_gate_review":
            return "Error: gate_review_json type must be research_option_comparison_gate_review"
        ...
        sections = [
            "## Research 方案对比门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名方案对比'}",
            f"- 推荐方案：{data.get('recommended_option') or '未说明'}",
            f"- 证据质量：{data.get('evidence_quality') or 'missing'}",
            f"- 候选方案数量：{data.get('option_count', 0)}",
            f"- 来源数量：{data.get('source_count', 0)}",
            f"- 一手来源：{data.get('primary_source_count', 0)}",
            f"- 边界：{data.get('note') or '这是方案对比进入计划或成文前的门禁，不代表已经完成实施验证。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
    ...
    def format_github_repo_risk_gate_review(
        gate_review_json: str,
        include_passed_items: bool = False,
    ) -> str:
        """把 github_repo_risk_gate_review JSON 转成可直接回复的中文摘要。"""
        ...
        if not isinstance(data, dict):
            return "Error: gate_review_json must be a JSON object"
        if data.get("type") != "github_repo_risk_gate_review":
        ...
        sections = [
            "## GitHub 仓库风险门禁审查",
            f"- 结论：{decision_labels.get(decision, decision or '待判断')}",
            f"- 审查对象：{data.get('review_target') or '未命名仓库'}",
            f"- 预期用途：{data.get('intended_action') or '未说明'}",
            f"- 原始风险判断：{data.get('source_decision') or 'unknown'}",
            f"- 边界：{data.get('note') or '这是仓库风险扫描门禁，不代表已经完成法律、安全或运行验证。'}",
            "",
            "## 未通过项",
            _markdown_table(["检查项", "状态", "依据"], failed_rows),
            "",
            "## 风险清单",
            _markdown_table(["级别", "领域", "问题", "缓解动作"], risk_rows),
            "",
            "## 下一步",
            _markdown_bullets(data.get("next_actions") if isinstance(data.get("next_actions"), list) else []),
        ]
    ...
    def render_collaboration_final_summary_gate_markdown(
        gate_review_json: str,
        summary_json: str = "",
        title: str = "",
        include_raw_metadata: bool = False,
    ) -> str:
        """把协作最终摘要门禁审查 JSON 渲染成正式 Markdown。"""
        ...
        if not isinstance(gate, dict):
            return "Error: gate_review_json must be a JSON object"
        if gate.get("type") != "collaboration_final_summary_gate_review":
            return "Error: gate_review_json type must be collaboration_final_summary_gate_review"
        ...
        if summary_json.strip():
            ...
            if not isinstance(parsed_summary, dict):
                return "Error: summary_json must be a JSON object"
            if parsed_summary.get("type") != "agent_collaboration_final_summary":
                return "Error: summary_json type must be agent_collaboration_final_summary"
            summary = parsed_summary

        review_target = str(gate.get("review_target") or summary.get("task_type") or "协作最终摘要").strip()
        decision = str(gate.get("decision") or "待审查").strip()
        completed = gate.get("completed_stage_count", summary.get("completed_stage_count", 0))
        total = gate.get("total_stage_count", summary.get("total_stage_count", 0))
        document_title = title.strip() or f"协作最终摘要门禁审查：{review_target}"
        summary_lines = [
            f"- 审查对象：{review_target}",
            f"- 门禁结论：{decision}",
            f"- 完成阶段：{completed} / {total}",
            f"- 最终结论：{summary.get('final_conclusion') or '待补充'}",
            "- 执行边界：这是协作最终摘要门禁审查，不代表任何 Agent 已经自动执行。",
        ]
        ...
        sections = [
            ...
            _markdown_section("摘要", "\n".join(summary_lines)),
            _markdown_section("门禁检查", _markdown_table(["检查项", "结果", "依据"], checklist_rows)),
            _markdown_section("未决事项", _markdown_bullets(gate.get("unresolved_items"))),
            _markdown_section("风险与限制", _markdown_bullets(gate.get("risks"))),
            _markdown_section("下一步", _markdown_bullets(gate.get("next_actions"))),
        ]
        if stage_rows:
            sections.insert(
                2,
                _markdown_section("关联阶段结果", _markdown_table(["步骤", "Agent", "状态", "摘要"], stage_rows)),
            )
        ...
        if include_raw_metadata:
            metadata = "\n\n" + _markdown_section(
                "结构化元数据",
                "```json\n"
                + json.dumps({"summary": summary, "gate_review": gate}, ensure_ascii=False, indent=2)
                + "\n```",
            )
    ...
    def classify_task_intent(user_text: str, context_hint: str = "") -> str:
        ...
        catalog = [
            ...
            },
            {
                ...
                "reason": "用户询问当前系统 Agent 能力目录，入口层可读取真实配置并格式化说明。",
                "next": "调用 list_agent_capabilities 读取真实能力目录，再调用 format_agent_capability_catalog 输出中文说明。",
                "direct": True,
                "requires_collaboration": False,
                "collaboration_task_type": "",
            },
            {
                "intent": "report-artifacts",
                "agent": "main",
                "keywords": (
                    "最近生成",
                    "生成了哪些报告",
                ...
                "reason": "用户询问已生成报告、可下载产物或附件路径，入口层可以只读查询报告产物索引。",
                "next": "调用 list_generated_reports，再调用 format_generated_report_list 输出中文报告产物索引。",
                "direct": True,
                "requires_collaboration": False,
                "collaboration_task_type": "",
            },
    ...
    registry.register(
        RegisteredTool(
            name="format_task_plan_gate_review",
            description=(
                "Format a task_plan_gate_review JSON object into a concise Chinese "
                "reviewer response with decision, failed checks, risks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    "gate_review_json": {
                        "type": "string",
                        "description": "JSON string returned by review_task_plan_gate.",
                    },
                    "include_passed_items": {
                        "type": "boolean",
                        "description": "Whether to include passed checklist items in the summary.",
            ...
            handler=format_task_plan_gate_review,
            tags=("review", "plan", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_agent_collaboration_gate_review",
            description=(
                "Format a collaboration_gate_review JSON object into a concise Chinese "
                "reviewer response with decision, agents, failed checks, risks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    "gate_review_json": {
                        "type": "string",
                        "description": "JSON string returned by review_agent_collaboration_gate.",
                    },
                    "include_passed_items": {
                        "type": "boolean",
                        "description": "Whether to include passed checklist items in the summary.",
            ...
            handler=format_agent_collaboration_gate_review,
            tags=("review", "agent", "collaboration", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_agent_handoff_package_gate_review",
            description=(
                "Format an agent_handoff_package_gate_review JSON object into a concise "
                "Chinese reviewer response with decision, failed checks, risks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
            ...
            handler=format_agent_handoff_package_gate_review,
            tags=("review", "agent", "handoff", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_collaboration_progress_gate_review",
            description=(
                "Format a collaboration_progress_gate_review JSON object into a concise "
                "Chinese reviewer response with progress, next stage, failed checks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
            ...
            handler=format_collaboration_progress_gate_review,
            tags=("review", "agent", "collaboration", "progress", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_research_evidence_gate_review",
            description=(
                "Format a research_evidence_gate_review JSON object into a concise Chinese "
                "reviewer response with decision, evidence quality, source counts, failed checks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    "gate_review_json": {
                        "type": "string",
                        "description": "JSON string returned by review_research_evidence_gate.",
                    },
                    "include_passed_items": {
                        "type": "boolean",
                        "description": "Whether to include passed checklist items in the summary.",
            ...
            handler=format_research_evidence_gate_review,
            tags=("review", "research", "evidence", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_research_option_comparison_gate_review",
            description=(
                "Format a research_option_comparison_gate_review JSON object into a concise "
                "Chinese reviewer response with decision, recommendation, source counts, "
                "failed checks, risks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    "gate_review_json": {
                        "type": "string",
                        "description": "JSON string returned by review_research_option_comparison_gate.",
                    },
                    "include_passed_items": {
                        "type": "boolean",
                        "description": "Whether to include passed checklist items in the summary.",
            ...
            handler=format_research_option_comparison_gate_review,
            tags=("review", "research", "comparison", "gate", "format", "user-facing"),
        )
    )
    registry.register(
        RegisteredTool(
    ...
    registry.register(
        RegisteredTool(
            name="format_github_repo_risk_gate_review",
            description=(
                "Format a github_repo_risk_gate_review JSON object into a concise Chinese "
                "reviewer response with decision, intended use, failed checks, risks, and next actions."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    "gate_review_json": {
                        "type": "string",
                        "description": "JSON string returned by review_github_repo_risk_gate.",
                    },
                    "include_passed_items": {
                        "type": "boolean",
                        "description": "Whether to include passed checklist items in the summary.",
    ...
    registry.register(
        RegisteredTool(
            name="render_collaboration_final_summary_gate_markdown",
            description=(
                "Render a collaboration_final_summary_gate_review JSON object from "
                "reviewer into a formal Chinese Markdown final-summary gate review report."
            ),
            input_schema={
                "type": "object",
                "required": ["gate_review_json"],
                "properties": {
                    ...
                    "summary_json": {
                        "type": "string",
                        "description": "Optional JSON string returned by compose_collaboration_final_summary.",
                    },
                    "title": {
                        "type": "string",
                        "description": "Optional Markdown H1 title.",
                    ...
                    "include_raw_metadata": {
                        "type": "boolean",
                        "description": "Whether to append raw final summary and gate review metadata.",
                    },
                },
            },
            handler=render_collaboration_final_summary_gate_markdown,
            tags=("document", "markdown", "agent", "collaboration", "summary", "gate", "review"),
        )
    )
    registry.register(
        RegisteredTool(
