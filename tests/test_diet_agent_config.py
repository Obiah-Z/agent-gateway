...
def test_diet_agent_config_is_user_scoped_without_owning_wework_entry() -> None:
    ...
    assert "weight_log_update_latest" in agent["tool_policy"]["tool_names"]
    assert "format_weight_log_update" in agent["tool_policy"]["tool_names"]
...
def test_main_agent_has_task_intent_classifier_and_prompt_boundary() -> None:
    ...
    assert "request_agent_handoff" not in tools
    ...
    assert "request_agent_handoff" not in combined_prompt
...
def test_platform_entry_agents_share_intent_classification_flow() -> None:
    ...
    for agent_id in ["feishu-entry", "wework-entry"]:
        ...
        assert "request_agent_handoff" not in identity
...
def test_diet_agent_prompt_requires_gender_inference() -> None:
    ...
    assert "weight_log_update_latest" in tools_md
    assert "weight_log_update_latest" in identity
    assert "weight_log_update_latest" in soul
    assert "format_weight_log_update" in tools_md
    assert "format_weight_log_update" in identity
    assert "format_weight_log_update" in soul
...
def test_platform_entry_agents_have_delegation_tool_only_at_entry_layer() -> None:
    ...
    assert "request_agent_handoff" not in tools["feishu-entry"]
    assert "request_agent_handoff" not in tools["wework-entry"]
    assert "request_agent_handoff" not in tools["main"]
    assert "request_agent_handoff" not in tools[SECRETARY_AGENT_ID]
    assert "start_agent_orchestration" in tools["feishu-entry"]
    assert "start_agent_orchestration" in tools["wework-entry"]
    assert "start_agent_orchestration" in tools["main"]
    assert "start_agent_orchestration" in tools[SECRETARY_AGENT_ID]
...
def test_personal_secretary_has_time_block_prompt_and_tool_rules() -> None:
    ...
    assert "personal_review_search" in identity
    assert "personal_review_search" in soul
    assert "personal_review_search" in tools_md
    ...
    assert "personal_action_closure_generate" in identity
    assert "personal_action_closure_generate" in soul
    assert "personal_action_closure_generate" in tools_md
    assert "format_personal_action_closure" in identity
    assert "format_personal_action_closure" in soul
    assert "format_personal_action_closure" in tools_md
    ...
    assert "personal_todo_status_card_generate" in identity
    assert "personal_todo_status_card_generate" in soul
    assert "personal_todo_status_card_generate" in tools_md
...
def test_personal_secretary_uses_orchestration_instead_of_one_shot_handoff() -> None:
    prompt_dir = ROOT / "workspace" / "agents" / SECRETARY_AGENT_ID
    identity = (prompt_dir / "IDENTITY.md").read_text(encoding="utf-8")
    soul = (prompt_dir / "SOUL.md").read_text(encoding="utf-8")
    tools_md = (prompt_dir / "TOOLS.md").read_text(encoding="utf-8")
    combined = "\n".join([identity, soul, tools_md])
