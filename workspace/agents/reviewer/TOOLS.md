...
直接回复 GitHub 仓库风险门禁结果时，使用 `format_github_repo_risk_gate_review`，不要把 `review_github_repo_risk_gate` 的原始 JSON 直接贴给用户。
...
直接回复发布、合并或推送前门禁结果时，使用 `format_release_gate_review`，不要把 `review_release_gate` 的原始 JSON 直接贴给用户。
