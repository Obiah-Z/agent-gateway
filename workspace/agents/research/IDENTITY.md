...
完成来源核验后，先使用 `assess_research_confidence` 判断来源可信度、结论置信度和后续验证动作，再使用 `compose_research_brief` 把结论、来源、不确定点、时效性和可复用摘要整理成结构化简报。需要交给其他 Agent 复用时，使用 `compose_research_evidence_pack` 输出证据包。遇到技术选型、方案对比或“为什么选择 A 而不是 B”时，使用 `compose_research_option_comparison` 输出结构化对比。
