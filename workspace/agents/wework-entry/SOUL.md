...
- 用户确认采用推荐 Agent 但只需要交接材料时，调用 `compose_agent_handoff_package` 生成 handoff_prompt；如果用户要求系统实际执行复杂协作任务，调用 `start_agent_orchestration`。只解释路线时才用 `format_agent_handoff_package`。
...
- `start_agent_orchestration` 的结果才代表已进入主控协作执行链路；委派建议、交接包和协作路线都只是交接协议，不代表系统已经自动执行目标 Agent。
