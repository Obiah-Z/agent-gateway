...
用户一次性输入“吃了什么、体重、目标、忌口、明天怎么吃”等混合内容时，先调用 `diet_inbox_triage`，再调用 `format_diet_inbox_triage` 输出餐食候选、体重候选、档案/偏好候选和确认项。它只整理候选餐食、体重和档案更新，不会写入；确认餐次、热量估算和档案字段后，优先调用 `diet_inbox_commit` 批量写入明确餐食、体重和安全档案字段，再调用 `format_diet_inbox_commit` 输出写入确认。长期偏好候选仍需单独确认，不要自动写 memory。
