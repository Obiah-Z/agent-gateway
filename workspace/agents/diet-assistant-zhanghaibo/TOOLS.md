...
用户查询“我的饮食档案”“还缺什么资料”“身高体重目标是否记录”时，先调用 `profile_get`。直接回复用户前，使用 `format_diet_profile` 转成中文 Markdown 摘要，不要直接输出原始 JSON。该查询只读取档案，不会修改餐食、体重或长期记忆。
...
用户查询今天吃了什么、记录了哪些餐或某天餐食明细时，调用 `meal_log_list`。直接回复用户前，使用 `format_meal_log_list` 转成中文 Markdown 摘要，不要直接输出原始 JSON。

用户查询今天热量或今日营养汇总时，调用 `nutrition_day_summary`。直接回复用户前，使用 `format_nutrition_day_summary` 转成中文 Markdown 摘要，不要直接输出原始 JSON。
...
用户询问“今天饮食状态”“今天吃得怎么样”“今天还差什么”“今天记录了什么”时，优先调用 `diet_today_status`。该工具只读取当天餐食、体重、计划和 7 天趋势，不会写入数据；直接回复用户前，使用 `format_diet_today_status` 转成中文 Markdown 状态卡，不要直接输出原始 JSON。
...
用户询问已保存饮食偏好、忌口、长期饮食习惯或“你记得我的饮食要求吗”时，先调用 `memory_search`，再调用 `format_memory_search` 转成中文摘要，不要直接输出带路径和 score 的内部检索文本。
